...
#: controls, and a workflow's `${globals.background}` reaches both the same way.
BACKGROUND_PARAMS: Tuple[Param, ...] = (
    Param(
        "background", str, "grid",
        "World-fixed environment drawn behind the frames, so an orbit reads as "
        "the camera moving rather than the subject spinning on a turntable. A "
        "built-in generator (grid, checker, blender_sky, gradient) or a path to "
        "an equirectangular image, a packed cubemap, or a directory of six cube "
        "faces. EMPTY disables it — which any render feeding "
        "select_support_views must do, since that step recovers a straight "
        "colour by dividing the alpha out and would recover the room instead",
    ),
    Param(
    ...
    Param(
        ...
        "backdrop's own appearance, as opposed to where its surface sits. Each "
        "generator names its own: grid takes base_color (the walls), "
        "line_color, floor_color and ceiling_color, plus n_per_face and "
        "line_width; checker takes color_a/color_b and n_per_face; gradient "
        "takes top_color/bottom_color; blender_sky takes zenith_color, "
        "horizon_color, ground_color and the sun_* angles. Every colour is RGB "
        "in [0,1], the same as bg_color and mesh_color. Written as a YAML "
        "mapping — {base_color: [0.20, 0.21, 0.24]} — and rejected, by name, "
        "if the generator does not accept a key. Must be empty when background "
        "is a path: a loaded image has no parameters to take",
    ),
    Param(
...
def build_background(params: Dict[str, Any], cameras: Sequence[Any]):
    ...
    background = Background.create(
        ...
        # Whole, unread. body2colmap checks these against the generator's own
        # signature and, on a miss, names every argument it does accept —
        # which is a better error than this project could write, and one that
        # cannot go stale when a generator gains a knob.
        params=params["background_params"] or None,
